...
if [ $OS == "Darwin" ]; then
    if [ -z "$(which pwsh)" ]; then 
        ...
        if [ -z "$(which brew)" ]; then
            ...
            cecho '[INFO] Installing PowerShell (Core) now...' blue
        ...
        cecho '[ OK ] PowerShell (Core) is found on your system.' green
        pwsh -NoProfile -ExecutionPolicy ByPass -Command "Invoke-Expression((New-Object System.Net.WebClient).DownloadString('https://raw.githubusercontent.com/ms-apac-csu/tools/main/Set-PSEnvironment.ps1'))"
    fi
elif [ $OS == "Linux" ]; then
    if [ -z "$(which pwsh)" ]; then
        cecho '[ NO ] No PowerShell (Core) is found on your system.' red
        sudo apt-get update
        sudo apt-get install -y wget apt-transport-https software-properties-common
        wget -q "https://packages.microsoft.com/config/ubuntu/$(lsb_release -rs)/packages-microsoft-prod.deb"
        sudo dpkg -i packages-microsoft-prod.deb
        sudo apt-get update
        cecho '[INFO] Installing PowerShell (Core) on your system now...' blue
